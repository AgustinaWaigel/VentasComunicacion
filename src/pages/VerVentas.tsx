...
export default function VerVentas() {
  ...
  const exportarExcelEvento = (nombreEvento: string, ventasDelEvento: Venta[]) => {
    ...
    ventasDelEvento.forEach((venta) => {
      ...
      const formaPago = venta.metodoPago === "transferencia" ? "Transferencia" : "Efectivo";
                  ...
                  {estadisticasPorEvento().map((stat) => (
                    <div key={stat.key} className="bg-gradient-to-br from-gray-50 to-white rounded-2xl p-6 border-2 border-gray-200 hover:shadow-lg transition-all duration-300">
                      <div className="flex items-center justify-between mb-4">
                        ...
                        <button
                          onClick={() => {
                            const ventasEvento = ventas.filter(v => {
                              if (stat.key === "campamento-adolescentes") return !v.evento_id;
                              return v.evento_id?.toString() === stat.key;
                            });
                            exportarExcelEvento(stat.nombre, ventasEvento);
                          }}
                          ...
                          title="Exportar Excel de este evento"
                        >
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
